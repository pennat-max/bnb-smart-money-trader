...
export async function POST(request: NextRequest) {
  const upstream = new URL("/api/paper/run", backendUrl);
  try {
    ...
    return NextResponse.json({ error: "backend_unreachable", message: error instanceof Error ? error.message : "Unknown backend fetch error" }, { status: 502 });
  }
}
